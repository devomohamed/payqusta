    ...
    <div className="space-y-8">
      <section className="space-y-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-xl font-bold app-text-body">{t('settings_system.ui.kh2hrel')}</h2>
            <p className="text-sm app-text-muted">{t('settings_system.ui.k15fjlr')}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div className="app-surface-muted rounded-2xl border border-[color:var(--surface-border)] p-5">
            <div className="mb-3 flex items-center gap-3">
              ...
              <h3 className="font-bold app-text-body">{t('settings_system.ui.ksaijkv')}</h3>
            </div>
            <p className="text-2xl font-black app-text-body">v{APP_VERSION}</p>
            <p className="mt-1 text-xs app-text-muted">{t('settings_system.ui.kvhqyjh')}</p>
          </div>

          <div className="app-surface-muted rounded-2xl border border-[color:var(--surface-border)] p-5">
            <div className="mb-3 flex items-center gap-3">
              <Hash className="h-5 w-5 text-emerald-500" />
              <h3 className="font-bold app-text-body">{t('settings_system.ui.ko6b11o')}</h3>
            </div>
            ...
            <p className="mt-2 text-[10px] uppercase tracking-tighter app-text-muted">{t('settings_system.ui.kfheqpj')}</p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-amber-200/70 bg-amber-50/70 p-5 dark:border-amber-900/20 dark:bg-amber-900/10">
          <div className="flex items-start gap-4">
            ...
            <div className="space-y-1">
              <h4 className="font-bold text-amber-800 dark:text-amber-200">{t('settings_system.ui.kl97j1q')}</h4>
              <p className="text-xs leading-relaxed text-amber-700/80 dark:text-amber-300/60">
                ...
                {t('settings_system.ui.kqe9xj')}
                {t('settings_system.ui.kwnlqpg')}
              </p>
            </div>
          </div>
        </div>
