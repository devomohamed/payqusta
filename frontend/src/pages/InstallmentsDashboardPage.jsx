                    ...
                    <tbody>
                      {visibleUpcoming.map((invoice) => (
                        <tr key={getInvoiceId(invoice)} className="border-b border-gray-100/70 font-medium transition-colors duration-200 hover:bg-primary-500/[0.03] dark:border-white/5 dark:text-gray-100 dark:hover:bg-white/[0.03]">
                          ...
                          <td className="px-4 py-3 dark:text-white">{invoice.customer?.name || 'عميل نقدي'}</td>
          ...
          <div className="space-y-4">
            ...
            <Card className="p-4">
              <div className="mb-3 flex items-center justify-between">
                <div>
                  <h3 className="font-bold dark:text-white">الديون الحالية</h3>
                  <p className="text-xs text-gray-500 dark:text-gray-400">الفلوس دي على أي فواتير، واتدفع منها كام، ولسه باقي كام.</p>
                </div>
            ...
            <Card className="p-4">
              <div className="mb-3 flex items-center justify-between">
                <div>
                  <h3 className="font-bold dark:text-white">تفاصيل الفواتير</h3>
                  <p className="text-xs text-gray-500 dark:text-gray-400">اضغط على الفاتورة لمعرفة البنود، سجل الدفعات، وجدول الأقساط.</p>
                </div>
                                ...
                                {invoice.installments.map((installment, index) => (
                                  <div key={`${invoice._id}-installment-${index}`} className="app-surface rounded-xl p-3">
                                    ...
                                    <div className="mt-2 grid grid-cols-3 gap-2 text-xs">
                                      <div><p className="text-gray-500 dark:text-gray-400">القيمة</p><p className="font-bold dark:text-white">{fmt(installment.amount)}</p></div>
                                      <div><p className="text-gray-500 dark:text-gray-400">المسدد</p><p className="font-bold text-emerald-600 dark:text-emerald-400">{fmt(installment.paidAmount)}</p></div>
                                      <div><p className="text-gray-500 dark:text-gray-400">المتبقي</p><p className="font-bold text-red-500 dark:text-red-400">{fmt(getInstallmentRemaining(installment))}</p></div>
                                    </div>
                                  </div>
                                ))}
                              </div>
