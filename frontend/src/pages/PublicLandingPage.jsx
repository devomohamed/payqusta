...
const spotlightCards = [
  ...
  },
  {
    ...
    text: 'موقع عام احترافي يعرف الزائر بالمشروع ويوصله مباشرة للخطوة التالية.',
  },
];
        ...
        <div className="grid gap-10 lg:grid-cols-[1.08fr,0.92fr] lg:items-center">
          <div className="text-right">
            ...
            <p className="mt-5 max-w-3xl text-lg leading-8 text-slate-600">
              بدل أن يرى الزائر شاشة دخول فقط، صار لدى {brandDisplayName} موقع عام متعدد الصفحات يشرح المشروع ويعرض قيمته
              الحقيقية. وفي الداخل، المنصة تربط بين البيع، المخزون، العملاء، الأقساط، والتحصيل في تجربة تشغيل واحدة.
            </p>

            <div className="mt-4 rounded-[1.5rem] border border-slate-200 bg-white/90 px-5 py-4 text-right shadow-sm">
            ...
            <div className="mt-8 flex flex-wrap justify-end gap-3">
              ...
              <Link
                to="/features"
                className="rounded-full border border-slate-300 bg-white px-6 py-3 text-sm font-black text-slate-800 transition-colors hover:border-slate-950 hover:text-slate-950"
              >
                استكشف المزايا
              </Link>
            </div>
            ...
            <div className="mt-8 flex flex-wrap justify-end gap-2">
              {platformHighlights.map((item) => (
                <span
                  key={item}
                  className="rounded-full border border-white bg-white px-4 py-2 text-xs font-bold text-slate-600 shadow-sm"
                >
                  {item}
                </span>
              ))}
          ...
          <div className="relative">
            ...
            <div className="relative overflow-hidden rounded-[2.25rem] border border-white/80 bg-slate-950 p-6 text-white shadow-[0_32px_90px_rgba(15,23,42,0.20)]">
              ...
              <div className="relative">
                ...
                <div className="mt-6 space-y-3">
                  {spotlightCards.map((card) => {
                    ...
                    return (
                      <div key={card.label} className="rounded-2xl border border-white/10 bg-white/8 p-4 text-right">
                        <div className="flex items-center justify-between">
                          <div className="text-right">
                            <p className="text-lg font-black">{card.label}</p>
                ...
                <div className="mt-5 rounded-[1.75rem] bg-white p-5 text-right text-slate-900">
                  ...
                  <p className="mt-2 text-sm leading-7 text-slate-600">
                    الموقع العام يشرح المشروع، والراوتس العامة تخدم الظهور والفهم، بينما النظام نفسه يظل عمليًا للاستخدام اليومي داخل النشاط.
                  </p>
                </div>
              </div>
            </div>
            ...
            {featurePillars.map((pillar) => (
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="text-sm font-black text-slate-500">{pillar.title}</p>
                    <p className="mt-2 text-base leading-7 text-slate-600">{pillar.summary}</p>
                  </div>
