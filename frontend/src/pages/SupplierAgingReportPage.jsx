...
function getAgingBuckets(t) {
    return [
        { key: 'current', label: t('supplier_aging_report_page.ui.kiw487e'), color: 'emerald' },
  ...
  const { t } = useTranslation('admin');
    ...
    const fetchAgingData = async () => {
        ...
        try {
            ...
            toast.error(t('supplier_aging_report_page.toasts.k9qm2w5'));
        } finally {
            setLoading(false);
        }
    };
    ...
    const exportCSV = () => {
        const headers = ['المورد', t('supplier_aging_report_page.ui.kaaw86k'), t('supplier_aging_report_page.ui.krh6w30'), '0-30', '31-60', '61-90', '91-120', '+120'];
        ...
        link.click();
        toast.success(t('supplier_aging_report_page.toasts.kkiubpe'));
    };

    if (loading) return <div className="flex flex-col items-center justify-center py-20 gap-4"><LoadingSpinner size="lg" /><p className="text-gray-500 animate-pulse">{t('supplier_aging_report_page.ui.kj5x7c0')}</p></div>;
            ...
            <section className="app-surface-muted overflow-hidden rounded-[2rem] border border-white/60 p-5 shadow-[0_24px_80px_-48px_rgba(15,23,42,0.5)] dark:border-white/10">
                <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <h1 className="text-2xl font-black text-gray-900 dark:text-white">{t('supplier_aging_report_page.ui.ky0zc2u')}</h1>
                            <p className="max-w-2xl text-sm leading-7 text-gray-500 dark:text-gray-400">
                                {t('supplier_aging_report_page.ui.k2dbju9')}
                            </p>
                        </div>
                    </div>
                ...
                <div className="mt-5 grid grid-cols-2 lg:grid-cols-6 gap-4">
                    <Card className="app-surface-muted p-4 border-primary-100 dark:border-primary-900/20">
                        <p className="text-[10px] uppercase tracking-wider font-bold text-primary-600 mb-1">{t('supplier_aging_report_page.ui.k8n6al6')}</p>
                        <p className="text-xl font-black text-gray-900 dark:text-white">{fmt(summary.total)} <span className="text-[10px]">{t('supplier_aging_report_page.ui.kwlxf')}</span></p>
                ...
                <div className="app-surface-muted flex flex-col items-center justify-between gap-4 border-b border-gray-100/80 p-4 dark:border-white/10 sm:flex-row">
                    <div className="relative w-full sm:w-64">
                        ...
                        <input
                            ...
                            placeholder={t('supplier_aging_report_page.placeholders.kggmahu')}
                            value={search}
                    ...
                    <div className="flex items-center gap-2 text-xs text-gray-400">
                        ...
                        <span>{t('supplier_aging_report_page.ui.kb4o4sd')}</span>
                    </div>
                </div>
                            ...
                            <div className="flex items-start justify-between gap-3">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="mt-1 text-xs text-gray-400">{supplier.phone || t('supplier_aging_report_page.toasts.kmn77dz')}</p>
                                    </div>
                                </div>
                                <div>
                                    {supplier.status === 'critical' ? <Badge variant="danger">{t('supplier_aging_report_page.ui.kt00q8')}</Badge> :
                                        supplier.status === 'warning' ? <Badge variant="warning">{t('supplier_aging_report_page.ui.kpbetmp')}</Badge> :
                                            supplier.status === 'delayed' ? <Badge variant="warning" className="opacity-70">{t('supplier_aging_report_page.ui.ktco0d')}</Badge> :
                                                <Badge variant="success">{t('supplier_aging_report_page.ui.kpbwupi')}</Badge>}
                                </div>
                            </div>

                            <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                                <div className="rounded-2xl bg-black/[0.03] p-3 dark:bg-white/[0.04]">
                                    <p className="text-[11px] text-gray-400">{t('supplier_aging_report_page.ui.krh6w30')}</p>
                                    <p className="mt-1 font-black text-gray-900 dark:text-white">{fmt(supplier.total)}</p>
                                </div>
                                <div className="rounded-2xl bg-black/[0.03] p-3 dark:bg-white/[0.04]">
                                    <p className="text-[11px] text-gray-400">{t('supplier_aging_report_page.ui.krrn8oz')}</p>
                        ...
                        <thead>
                            <tr className="app-surface-muted text-[11px] font-bold uppercase tracking-wider text-gray-400">
                                <th className="px-6 py-4 cursor-pointer hover:text-primary-500" onClick={() => toggleSort('name')}>
                                    <div className="flex items-center gap-2">{t('supplier_aging_report_page.ui.kaawtj6')} <SortIcon field="name" /></div>
                                </th>
                                <th className="px-6 py-4 cursor-pointer hover:text-primary-500 text-center" onClick={() => toggleSort('total')}>
                                    <div className="flex items-center justify-center gap-2">{t('supplier_aging_report_page.ui.krh6w30')} <SortIcon field="total" /></div>
                                </th>
                                ...
                                <th className="px-6 py-4 text-center">{t('supplier_aging_report_page.ui.kabct8k')}</th>
                            </tr>
                        </thead>
                            ...
                            {paginated.map(s => (
                                <tr key={s._id} className="group transition-colors hover:bg-black/[0.02] dark:hover:bg-white/[0.03]">
                                    ...
                                    <td className="px-6 py-4 text-center">
                                        {s.status === 'critical' ? <Badge variant="danger">{t('supplier_aging_report_page.ui.kt00q8')}</Badge> :
                                            s.status === 'warning' ? <Badge variant="warning">{t('supplier_aging_report_page.ui.kpbetmp')}</Badge> :
                                                s.status === 'delayed' ? <Badge variant="warning" className="opacity-70">{t('supplier_aging_report_page.ui.ktco0d')}</Badge> :
                                                    <Badge variant="success">{t('supplier_aging_report_page.ui.kpbwupi')}</Badge>}
                                    </td>
                                </tr>
                            ))}
