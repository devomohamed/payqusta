...
const getOrderStatusOptions = (t) => [
    { value: '', label: t('portal_orders_admin_page.ui.k94l6ci') },
    { value: 'pending', label: t('portal_orders_admin_page.ui.k40qeml') },
    { value: 'confirmed', label: t('portal_orders_admin_page.ui.kteb63') },
    { value: 'processing', label: t('portal_orders_admin_page.ui.k3boezg') },
    { value: 'shipped', label: t('portal_orders_admin_page.ui.kwttx6d') },
    { value: 'delivered', label: t('portal_orders_admin_page.ui.kar3bik') },
    { value: 'cancelled', label: t('portal_orders_admin_page.ui.kteypb') },
];

const getStatusConfig = (t) => ({
    pending: { label: t('portal_orders_admin_page.ui.k40qeml'), color: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400', icon: Clock },
    confirmed: { label: t('portal_orders_admin_page.ui.kteb63'), color: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400', icon: CheckCircle },
    processing: { label: t('portal_orders_admin_page.ui.k3boezg'), color: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-400', icon: Package },
    shipped: { label: t('portal_orders_admin_page.ui.kwttx6d'), color: 'bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400', icon: Truck },
    delivered: { label: t('portal_orders_admin_page.ui.kar3bik'), color: 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400', icon: CheckCircle },
    cancelled: { label: t('portal_orders_admin_page.ui.kteypb'), color: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400', icon: XCircle },
});
  ...
  const statusConfig = useMemo(() => getStatusConfig(t), [t]);
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            notify.error(t('portal_orders_admin_page.toasts.k2qfpyq'));
        } finally {
            setLoading(false);
        }
    ...
    const updateStatus = async (orderId, newStatus) => {
        ...
        try {
            ...
            notify.success(t('portal_orders_admin_page.toasts.kyosw45'));
            load();
            if (selected?._id === orderId) setSelected(prev => ({ ...prev, orderStatus: newStatus }));
        } catch {
            notify.error(t('portal_orders_admin_page.toasts.kjwcs3i'));
        } finally {
            setUpdatingId(null);
        }
    };
    ...
    const createShipment = async (orderId) => {
        ...
        try {
            ...
            notify.success(`تم إنشاء الشحنة: ${res.data?.data?.waybillNumber || t('portal_orders_admin_page.toasts.kwt90pd')}`);
            await load();
            await refreshSelectedOrder(orderId);
        } catch (err) {
            notify.error(err.response?.data?.message || t('portal_orders_admin_page.toasts.k80fior'));
        } finally {
    ...
    const syncShipment = async (orderId) => {
        ...
        try {
            ...
            notify.success(`تم تحديث الشحنة إلى: ${res.data?.data?.status || t('portal_orders_admin_page.toasts.k4ymf61')}`);
            await load();
            await refreshSelectedOrder(orderId);
        } catch (err) {
            notify.error(err.response?.data?.message || t('portal_orders_admin_page.toasts.kbwin14'));
        } finally {
            setShippingActionId(null);
        }
    };
    ...
    const processRefund = async (orderId) => {
        ...
        try {
            ...
            if (refund.executedAmount > 0) {
                ...
                notify.success(res.data?.message || t('portal_orders_admin_page.toasts.kkqbabi'));
            }
            await load();
            await refreshSelectedOrder(orderId);
        } catch (err) {
            notify.error(err.response?.data?.message || t('portal_orders_admin_page.toasts.kk9gm0z'));
        } finally {
        ...
        <div className="space-y-6 animate-fade-in app-text-soft">
            ...
            <div className="app-surface-muted flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-5 rounded-3xl">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-0.5">{t('portal_orders_admin_page.ui.kf0jky8')}</p>
                </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
                <div className="relative flex-1">
                    ...
                    <input
                        ...
                        placeholder={t('portal_orders_admin_page.placeholders.k7zksoh')}
            ...
            {loading ? <LoadingSpinner /> : orders.length === 0 ? (
                <EmptyState
                    ...
                    title={t('portal_orders_admin_page.titles.kk4v7to')}
                    description={statusFilter ? t('portal_orders_admin_page.ui.k9ldyvy') : 'لم يتم استلام أي طلبات من البوابة بعد'}
                />
            ) : (
                <>
                            ...
                            <table className="w-full text-sm text-right">
                                <thead>
                                    <tr className="app-surface-muted border-b border-gray-100/80 dark:border-white/10">
                                        <th className="px-5 py-4 font-bold text-gray-500 dark:text-gray-400">{t('portal_orders_admin_page.ui.kig6vo2')}</th>
                                        <th className="px-5 py-4 font-bold text-gray-500 dark:text-gray-400">{t('portal_orders_admin_page.ui.kab4izh')}</th>
                                        <th className="px-5 py-4 font-bold text-gray-500 dark:text-gray-400">{t('portal_orders_admin_page.ui.kzcgj78')}</th>
                                        <th className="px-5 py-4 font-bold text-gray-500 dark:text-gray-400">{t('portal_orders_admin_page.ui.kzbvdnf')}</th>
                                        <th className="px-5 py-4 font-bold text-gray-500 dark:text-gray-400">{t('portal_orders_admin_page.ui.krh6w30')}</th>
                                        <th className="px-5 py-4 font-bold text-gray-500 dark:text-gray-400">{t('portal_orders_admin_page.ui.kabct8k')}</th>
                                        <th className="px-5 py-4 font-bold text-gray-500 dark:text-gray-400 text-center">{t('portal_orders_admin_page.ui.k5a5wt5')}</th>
                                    </tr>
                                </thead>
                                    ...
                                    {orders.map(order => (
                                        <tr key={order._id} className="group transition-colors hover:bg-black/[0.02] dark:hover:bg-white/[0.03]">
                                            ...
                                            <td className="px-5 py-4 font-black text-gray-800 dark:text-gray-200">
                                                {fmt(order.totalAmount)} <span className="text-[10px] font-normal text-gray-400">{t('portal_orders_admin_page.ui.kwlxf')}</span>
                                            </td>
                                            <td className="px-5 py-4">
                                            ...
                                            <td className="px-5 py-4">
                                                <div className="flex items-center justify-center gap-2">
                                                    <button
                                                        ...
                                                        title={t('portal_orders_admin_page.titles.k3y9kzm')}
                                                    >
                                                    ...
                                                    {NEXT_STATUS[order.orderStatus] && (
                                                        <button
                                                            ...
                                                            title={`تحديث إلى: ${statusConfig[NEXT_STATUS[order.orderStatus]]?.label}`}
                                                        >
                                                    ...
                                                    {!order.shippingDetails?.waybillNumber && order.orderStatus !== 'cancelled' ? (
                                                        <button
                                                            ...
                                                            title={t('portal_orders_admin_page.titles.kydksq7')}
                                                        >
                                                    ...
                                                    {order.shippingDetails?.waybillNumber ? (
                                                        <button
                                                            ...
                                                            title={t('portal_orders_admin_page.titles.kd5n2wc')}
                                                        >
                                                    ...
                                                    {['pending', 'confirmed', 'processing'].includes(order.orderStatus) && (
                                                        <button
                                                            ...
                                                            title={t('portal_orders_admin_page.titles.kuqz2jx')}
                                                        >
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            {NEXT_STATUS[selected.orderStatus] && (
                                ...
                                >
                                    {updatingId === selected._id ? <RefreshCw className="w-4 h-4 animate-spin" /> : <><CheckCircle className="w-4 h-4" /> تحديث إلى: {statusConfig[NEXT_STATUS[selected.orderStatus]]?.label}</>}
                                </button>
                            )}
                            ...
                            {selected.shippingDetails?.waybillNumber ? (
                                ...
                                >
                                    ...
                                    {t('portal_orders_admin_page.ui.kd5n2wc')}
                                </button>
                            ) : null}
                            {selected.refundStatus && ['pending', 'partially_refunded', 'failed'].includes(selected.refundStatus) && Number(selected.refundAmount || 0) > 0 ? (
                                ...
                                >
                                    ...
                                    {t('portal_orders_admin_page.ui.k59fy6t')}
                                </button>
                            ) : null}
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="app-surface-muted rounded-2xl p-4">
                                ...
                                <div className="space-y-2 text-sm">
                                    <div className="flex justify-between"><span className="text-gray-400">{t('portal_orders_admin_page.ui.kovdol8')}</span><span className="font-bold">{selected.customer?.name}</span></div>
                                    <div className="flex justify-between"><span className="text-gray-400">{t('portal_orders_admin_page.ui.ku7pslo')}</span><span className="font-bold" dir="ltr">{selected.customer?.phone}</span></div>
                                </div>
                            </div>
                                <div className="app-surface-muted rounded-2xl p-4">
                                ...
                                {selected.shippingAddress ? (
                                    <div className="space-y-2 text-sm">
                                        <div className="flex justify-between"><span className="text-gray-400">{t('portal_orders_admin_page.ui.kza6ps0')}</span><span className="font-bold">{selected.shippingAddress.fullName}</span></div>
                                        <div className="flex justify-between"><span className="text-gray-400">{t('portal_orders_admin_page.ui.ku7pslo')}</span><span className="font-bold" dir="ltr">{selected.shippingAddress.phone}</span></div>
                                        <div className="flex justify-between"><span className="text-gray-400">{t('portal_orders_admin_page.ui.kseg3bk')}</span><span className="font-bold">{selected.shippingAddress.governorate} {selected.shippingAddress.city && `/ ${selected.shippingAddress.city}`}</span></div>
                                        <div className="flex justify-between"><span className="text-gray-400 flex-shrink-0">{t('portal_orders_admin_page.ui.kzgfilf')}</span><span className="font-bold text-left max-w-[60%]">{selected.shippingAddress.address}</span></div>
                                        {selected.shippingAddress.notes && <div className="flex justify-between"><span className="text-gray-400">{t('portal_orders_admin_page.ui.notes')}</span><span className="text-gray-600 dark:text-gray-400 text-left max-w-[60%]">{selected.shippingAddress.notes}</span></div>}
                                    </div>
                                ) : (
                                    <EmptyState
                                        icon={MapPin}
                                        title={t('portal_orders_admin_page.titles.k3nhhbd')}
                            ...
                            <div className="app-surface-muted rounded-2xl p-4">
                                ...
                                <div className="space-y-2 text-sm">
                                    <div className="flex justify-between"><span className="text-gray-400">{t('portal_orders_admin_page.ui.krmcqvo')}</span><span className="font-bold">{selected.shippingMethod || '—'}</span></div>
                                    <div className="flex justify-between"><span className="text-gray-400">{t('portal_orders_admin_page.ui.k3os116')}</span><span className="font-bold" dir="ltr">{selected.trackingNumber || selected.shippingDetails?.waybillNumber || '—'}</span></div>
                                    <div className="flex justify-between"><span className="text-gray-400">{t('portal_orders_admin_page.ui.ke7gqic')}</span><span className="font-bold">{selected.shippingDetails?.status || '—'}</span></div>
                                    <div className="flex justify-between"><span className="text-gray-400">{t('portal_orders_admin_page.ui.k3bxnv1')}</span><span className="font-bold">{selected.estimatedDeliveryDate ? new Date(selected.estimatedDeliveryDate).toLocaleDateString('ar-EG') : '—'}</span></div>
                                </div>
                                {selected.shippingDetails?.trackingUrl ? (
                                    ...
                                    >
                                        {t('portal_orders_admin_page.ui.ke75icz')}
                                    </button>
                                ) : null}
                            </div>
                            {(selected.refundStatus && selected.refundStatus !== 'none') || selected.cancelReason ? (
                            <div className="app-surface-muted rounded-2xl p-4">
                                    <h4 className="font-bold text-xs text-gray-400 uppercase mb-3">{t('portal_orders_admin_page.ui.kkobf6x')}</h4>
                                    <div className="space-y-2 text-sm">
                                        {selected.refundStatus && selected.refundStatus !== 'none' ? (
                                            <div className="flex justify-between">
                                                <span className="text-gray-400">{t('portal_orders_admin_page.ui.kl1zpa2')}</span>
                                                <span className="font-bold text-emerald-700 dark:text-emerald-300">
                                                    {selected.refundStatus === 'pending'
                                                        ? t('portal_orders_admin_page.ui.ki0w2sk') : selected.refundStatus === 'partially_refunded'
                                                            ? t('portal_orders_admin_page.ui.kful99v') : selected.refundStatus === 'refunded'
                                                                ? t('portal_orders_admin_page.ui.khyyhdf') : selected.refundStatus === 'failed'
                                                                    ? t('portal_orders_admin_page.ui.khokel4') : selected.refundStatus}
                                                </span>
                                            </div>
                                        ) : null}
                                        {Number(selected.refundAmount || 0) > 0 ? (
                                            <div className="flex justify-between">
                                                <span className="text-gray-400">{t('portal_orders_admin_page.ui.kh8ks6k')}</span>
                                                <span className="font-bold">{fmt(selected.refundAmount)} ج.م</span>
                                            </div>
                                        ) : null}
                                        {selected.cancelReason ? (
                                            <div className="flex justify-between gap-3">
                                                <span className="text-gray-400 flex-shrink-0">{t('portal_orders_admin_page.ui.kbrs3jf')}</span>
                        ...
                        <div>
                            ...
                            <div className="flex justify-between items-center mt-3 pt-3 border-t border-gray-100/80 dark:border-white/10 px-4">
                                <span className="font-bold text-gray-700 dark:text-gray-300">{t('portal_orders_admin_page.ui.krh6w30')}</span>
                        ...
                        {selected.notes && (
                            <div className="bg-amber-50 dark:bg-amber-900/20 rounded-xl p-4 text-sm text-amber-700 dark:text-amber-400">
                                <span className="font-bold">{t('portal_orders_admin_page.ui.kua4x18')} </span>{selected.notes}
                            </div>
                        )}
                        ...
                        {selected.electronicSignature && (
                            <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-xl p-4 text-sm text-emerald-700 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-800">
                                <h4 className="font-bold mb-1 flex items-center gap-2">{t('portal_orders_admin_page.ui.kexeo1r')}</h4>
                                <p className="text-emerald-600 dark:text-emerald-500 text-xs mb-2">
                                    {t('portal_orders_admin_page.ui.kiqcjsb')}
                                </p>
                        ...
                        {selected.orderStatusHistory?.length > 0 && (
                            <div>
                                <h4 className="font-bold text-xs text-gray-400 uppercase mb-3">{t('portal_orders_admin_page.ui.kdacimn')}</h4>
                                <div className="space-y-2">
                                    {selected.orderStatusHistory.map((h, i) => (
                                        <div key={i} className="flex items-center gap-3 text-xs">
                                            <span className={`px-2 py-0.5 rounded-full font-bold ${statusConfig[h.status]?.color || 'bg-gray-100 text-gray-600'}`}>{statusConfig[h.status]?.label || h.status}</span>
