      ...
      <div className="bg-white dark:bg-gray-800 rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-extrabold text-xl text-gray-900 dark:text-white">{t('admin_shifts_page.ui.kicps7i')}</h3>
              <p className="text-xs text-gray-500 font-medium">بواسطة: {shift.user?.name || t('admin_shifts_page.toasts.kpmv4w1')}</p>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto">
                 ...
                 <strong className="text-sm">
                    {shift.endTime ? (
                      <div className="flex items-center gap-2">
                        ...
                        {shift.closedBySystem && <Badge variant="danger" className="text-[10px]">{t('admin_shifts_page.ui.kk352sn')}</Badge>}
                      </div>
                    ) : <span className="text-emerald-500">{t('admin_shifts_page.ui.kymsru4')}</span>}
                 </strong>
              </div>
           </div>
           ...
           <div className="space-y-4">
              <div className="flex items-center justify-between border-b border-gray-100 dark:border-gray-700 pb-2">
                 <h4 className="font-bold text-gray-900 dark:text-gray-100">{t('admin_shifts_page.ui.kf36bvm')}</h4>
                 <div className="bg-primary-50 dark:bg-primary-900/20 px-3 py-1 rounded-full border border-primary-100 dark:border-primary-900/30">
                    <span className="text-xs font-bold text-primary-600 dark:text-primary-400">{shift.branch?.name || shift.user?.branch?.name || t('admin_shifts_page.toasts.kphehwb')}</span>
                 </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                 <div className="flex justify-between p-3 bg-blue-50/50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-900/20 transition-colors hover:bg-blue-50">
                    <span className="text-xs font-semibold text-blue-700 dark:text-blue-400">{t('admin_shifts_page.ui.kbu3ryl')}</span>
                 ...
                 <div className="flex justify-between p-3 bg-emerald-50/50 dark:bg-emerald-900/10 rounded-xl border border-emerald-100 dark:border-emerald-900/20 transition-colors hover:bg-emerald-50">
                    <span className="text-xs font-semibold text-emerald-700 dark:text-emerald-400">{t('admin_shifts_page.ui.kflwesj')}</span>
                 ...
                 <div className="flex justify-between p-4 bg-purple-50 dark:bg-purple-900/10 rounded-2xl border border-purple-100 dark:border-purple-900/20 col-span-2 shadow-sm">
                    <span className="text-sm font-bold text-purple-700 dark:text-purple-400">{t('admin_shifts_page.ui.ko82li0')}</span>
              ...
              {shift.status === 'closed' && (
                 <>
                   <h4 className="font-bold text-gray-900 dark:text-gray-100 border-b border-gray-100 dark:border-gray-700 pb-2 mt-6">{t('admin_shifts_page.ui.k2jsb4')}</h4>
                   <div className="grid grid-cols-2 gap-3">
                      <div className="flex justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700">
                         <span className="text-xs font-semibold text-gray-600 dark:text-gray-400">{t('admin_shifts_page.ui.kpf3s66')}</span>
                      ...
                      <div className={`flex justify-between p-3 rounded-xl border ${shift.variance < 0 ? 'bg-rose-50 border-rose-200 text-rose-700' : shift.variance > 0 ? 'bg-amber-50 border-amber-200 text-amber-700' : 'bg-emerald-50 border-emerald-200 text-emerald-700'}`}>
                         <span className="text-xs font-semibold flex items-center gap-1">
                            ...
                            {t('admin_shifts_page.ui.kqbmuot')}
                         </span>
                   ...
                   {shift.notes && (
                      <div className="mt-4 p-4 bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-gray-100 dark:border-gray-800">
                         <h5 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2 flex items-center gap-2">
                           ...
                           {t('admin_shifts_page.ui.khb89r1')}
                         </h5>
...
export default function AdminShiftsPage() {
  ...
  const fetchShifts = async () => {
    ...
    try {
      ...
      toast.error(t('admin_shifts_page.toasts.kc982uj'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchAnalytics = async () => {
    ...
    try {
      ...
      toast.error(t('admin_shifts_page.toasts.kc6rslw'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleForceClose = async () => {
    ...
    try {
      ...
      toast.success(t('admin_shifts_page.toasts.k8kjqfb'));
      setForceCloseModal(null);
      fetchShifts();
    } catch (err) {
      toast.error(err.response?.data?.message || t('admin_shifts_page.toasts.kvcbffh'));
    }
  };
      ...
      <section className="overflow-hidden rounded-[1.75rem] border border-white/40 bg-gradient-to-br from-slate-950 via-primary-800 to-violet-700 px-5 py-6 text-white shadow-[0_30px_80px_-46px_rgba(79,70,229,0.85)] sm:px-6">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-4 text-2xl font-black sm:text-3xl">{t('admin_shifts_page.ui.kdybllo')}</h1>
            <p className="mt-2 max-w-2xl text-sm leading-7 text-white/80">{t('admin_shifts_page.ui.kfrx8nn')}</p>
          </div>

          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4 xl:min-w-[620px]">
            <div className="rounded-2xl border border-white/10 bg-white/10 px-4 py-4 backdrop-blur-sm">
              <p className="text-xs font-bold text-white/65">{t('admin_shifts_page.ui.kmsd2hd')}</p>
              <p className="mt-2 text-2xl font-black">{openCount}</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/10 px-4 py-4 backdrop-blur-sm">
              <p className="text-xs font-bold text-white/65">{t('admin_shifts_page.ui.kuiwrr7')}</p>
              <p className="mt-2 text-2xl font-black">{closedCount}</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/10 px-4 py-4 backdrop-blur-sm">
              <p className="text-xs font-bold text-white/65">{t('admin_shifts_page.ui.kflwesj')}</p>
              <p className="mt-2 text-lg font-black">{fmt(totalSales)} ج.م</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/10 px-4 py-4 backdrop-blur-sm">
              <p className="text-xs font-bold text-white/65">{t('admin_shifts_page.ui.kqbmuot')}</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
         <Card className="p-5 border-l-4 border-l-primary-500 bg-white dark:bg-gray-900">
            <div className="flex items-center gap-4">
               ...
               <div>
                  <p className="text-xs text-gray-500 font-bold mb-1">{t('admin_shifts_page.ui.kna8s29')}</p>
         ...
         <Card className="p-5 border-l-4 border-l-gray-400 bg-white dark:bg-gray-900">
            <div className="flex items-center gap-4">
               ...
               <div>
                  <p className="text-xs text-gray-500 font-bold mb-1">{t('admin_shifts_page.ui.kju2mtz')}</p>
         ...
         <Card className="p-5 border-l-4 border-l-green-500 bg-white dark:bg-gray-900">
            <div className="flex items-center gap-4">
               ...
               <div>
                  <p className="text-xs text-gray-500 font-bold mb-1">{t('admin_shifts_page.ui.krno97g')}</p>
                  <h3 className="text-xl font-black text-green-600 dark:text-green-400">{fmt(totalSales)} <span className="text-xs text-gray-500 font-normal">{t('admin_shifts_page.ui.kwlxf')}</span></h3>
               </div>
            </div>
         </Card>
         <Card className={`p-5 border-l-4 bg-white dark:bg-gray-900 ${totalVariance < 0 ? 'border-l-red-500' : 'border-l-amber-500'}`}>
            <div className="flex items-center gap-4">
               ...
               <div>
                  <p className="text-xs text-gray-500 font-bold mb-1">{t('admin_shifts_page.ui.kqnw030')}</p>
                  <h3 className={`text-xl font-black ${totalVariance < 0 ? 'text-red-600' : totalVariance > 0 ? 'text-amber-600' : 'text-gray-900 dark:text-white'}`} dir="ltr">{totalVariance > 0 ? '+' : ''}{fmt(totalVariance)} <span className="text-xs text-gray-500 font-normal" dir="rtl">{t('admin_shifts_page.ui.kwlxf')}</span></h3>
               </div>
            </div>
         </Card>
      </div>
      ...
      {shifts.filter(s => s.status === 'open').length > 0 && (
        <section className="space-y-4">
          <div className="flex items-center gap-2 px-2">
            ...
            <h2 className="text-lg font-black text-gray-800 dark:text-gray-200">{t('admin_shifts_page.ui.kxevefz')}</h2>
          </div>
              ...
              <Card key={shift._id} className="p-4 border-emerald-100 dark:border-emerald-900/30 hover:shadow-lg transition-all cursor-pointer group relative overflow-hidden" onClick={() => setSelectedShift(shift)}>
                ...
                <div className="flex items-start justify-between mb-4 relative z-10">
                   <div className="flex items-center gap-3">
                      ...
                      <div>
                         <h4 className="font-black text-gray-900 dark:text-white line-clamp-1">{shift.branch?.name || shift.user?.branch?.name || t('admin_shifts_page.toasts.kphehwb')}</h4>
                         <p className="text-[10px] text-gray-400 font-bold">{shift.user?.name}</p>
                      </div>
                   </div>
                   <Badge className="bg-emerald-500 text-white border-none animate-pulse-subtle text-[9px] px-2 py-0.5">{t('admin_shifts_page.ui.ky62x')}</Badge>
                </div>

                <div className="space-y-2 relative z-10">
                   <div className="flex justify-between items-center text-xs">
                      <span className="text-gray-500">{t('admin_shifts_page.ui.kteyqz')}</span>
                      <span className="font-bold text-gray-700 dark:text-gray-300">{shiftDuration(shift.startTime)}</span>
                   </div>
                   <div className="flex justify-between items-center">
                      <span className="text-xs text-gray-500">{t('admin_shifts_page.ui.kbpdnmk')}</span>
                      <span className="font-black text-emerald-600 dark:text-emerald-400 text-sm">+{fmt(shift.totalCashSales)} <small className="text-[10px]">{t('admin_shifts_page.ui.kwlxf')}</small></span>
                   </div>
                    <div className="pt-2 border-t border-gray-100 dark:border-gray-800 flex justify-between items-center gap-2">
                       <span className="text-xs font-bold text-gray-400">{t('admin_shifts_page.ui.ktc2dci')}</span>
                       <div className="flex items-center gap-2">
                          ...
                          <button 
                            ...
                            title={t('admin_shifts_page.titles.kpxtrb4')}
                          >
        ...
        <div className="p-4 border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/20 flex flex-wrap gap-4 items-center justify-between">
           <div className="flex items-center gap-3 w-full sm:w-auto">
              <div className="flex items-center bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-1 shadow-sm">
                 <button onClick={() => setStatusFilter('')} className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all ${statusFilter === '' ? 'bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white' : 'text-gray-500 hover:text-gray-700'}`}>{t('admin_shifts_page.ui.ksvtb2')}</button>
                 <button onClick={() => setStatusFilter('open')} className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all ${statusFilter === 'open' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : 'text-gray-500 hover:text-gray-700'}`}>{t('admin_shifts_page.ui.kaawl8j')}</button>
                 <button onClick={() => setStatusFilter('closed')} className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all ${statusFilter === 'closed' ? 'bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-gray-200' : 'text-gray-500 hover:text-gray-700'}`}>{t('admin_shifts_page.ui.kza1pnr')}</button>
              </div>
               ...
               >
                  <option value="">{t('admin_shifts_page.ui.kai1sru')}</option>
            ...
            <div className="flex items-center bg-gray-100 dark:bg-gray-800 p-1 rounded-xl shadow-inner">
               <button onClick={() => setActiveTab('history')} className={`px-5 py-1.5 rounded-lg text-sm font-extrabold transition-all ${activeTab === 'history' ? 'bg-white dark:bg-gray-700 text-primary-600 dark:text-primary-400 shadow-sm' : 'text-gray-500'}`}>{t('admin_shifts_page.ui.khno5q9')}</button>
               <button onClick={() => setActiveTab('analytics')} className={`px-5 py-1.5 rounded-lg text-sm font-extrabold transition-all ${activeTab === 'analytics' ? 'bg-white dark:bg-gray-700 text-primary-600 dark:text-primary-400 shadow-sm' : 'text-gray-500'}`}>{t('admin_shifts_page.ui.ksk9br6')}</button>
            </div>
           ...
           {activeTab === 'history' ? (
              ...
              <div className="space-y-3 p-4 md:hidden">
                {shifts.length === 0 && !loading ? (
                  <EmptyState title={t('admin_shifts_page.titles.km3iafu')} description="لم يتم العثور على ورديات تطابق بحثك" />
                ) : (
                  shifts.map((shift) => (
                    <div key={shift._id} className="app-surface-muted rounded-2xl p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="text-sm font-black text-gray-900 dark:text-white">{shift.branch?.name || shift.user?.branch?.name || t('admin_shifts_page.toasts.kphehwb')}</p>
                          <p className="mt-1 text-[11px] text-gray-400">{shift.user?.name}</p>
                        </div>
                        {shift.status === 'open' ? (
                          <Badge className="bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400">{t('admin_shifts_page.ui.kd4yj7j')}</Badge>
                        ) : (
                          <Badge className="bg-gray-100 text-gray-600 dark:bg-gray-800">{t('admin_shifts_page.ui.kpbpqd2')}</Badge>
                        )}
                      </div>
                      <div className="mt-3 grid grid-cols-2 gap-2">
                        <div className="rounded-xl bg-white px-3 py-2 text-center dark:bg-gray-900/70">
                          <p className="text-[10px] text-gray-400">{t('admin_shifts_page.ui.koveapu')}</p>
                          <p className="mt-1 text-xs font-black text-gray-900 dark:text-white">{shiftDuration(shift.startTime, shift.endTime)}</p>
                        </div>
                        <div className="rounded-xl bg-white px-3 py-2 text-center dark:bg-gray-900/70">
                          <p className="text-[10px] text-gray-400">{t('admin_shifts_page.ui.ksgkw32')}</p>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        ...
                        >
                          {t('admin_shifts_page.ui.ku5ftfg')}
                        </button>
                        {shift.status === 'open' && (
                          <button
                          ...
                          >
                            {t('admin_shifts_page.ui.kpxtrb4')}
                          </button>
                        )}
                      </div>
                    </div>
              ...
              <table className="w-full text-sm text-right">
                 ...
                 <thead className="bg-gray-50/80 dark:bg-gray-800/50 text-gray-500 dark:text-gray-400 font-bold border-b border-gray-100 dark:border-gray-800">
                    <tr>
                       <th className="px-6 py-4">{t('admin_shifts_page.ui.kabct8k')}</th>
                       <th className="px-6 py-4">{t('admin_shifts_page.ui.k3hhfat')}</th>
                       <th className="px-6 py-4">{t('admin_shifts_page.ui.koveapu')}</th>
                       <th className="px-6 py-4">{t('admin_shifts_page.ui.ksgkw32')}</th>
                       <th className="px-6 py-4">{t('admin_shifts_page.ui.kqbmuot')}</th>
                       <th className="px-6 py-4 text-center">{t('admin_shifts_page.ui.kvfmk6')}</th>
                    </tr>
                 </thead>
                        ...
                        <td className="px-6 py-4">
                              {shift.status === 'open' ? (
                                 <Badge className="bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 flex items-center justify-center gap-1.5 w-fit font-bold border-emerald-200 dark:border-emerald-800">
                                    ...
                                    {t('admin_shifts_page.ui.kd4yj7j')}
                                 </Badge>
                              ) : (
                                 <div className="flex flex-col gap-1">
                                   <Badge className="bg-gray-100 text-gray-600 dark:bg-gray-800 flex items-center justify-center w-fit font-bold">
                                      {t('admin_shifts_page.ui.kpbpqd2')}
                                   </Badge>
                                   {shift.closedBySystem && (
                                     <span className="text-[9px] font-bold text-rose-500 bg-rose-50 dark:bg-rose-900/20 px-1.5 py-0.5 rounded-full w-fit">{t('admin_shifts_page.ui.kk352sn')}</span>
                                   )}
                                 </div>
                              )}
                           </td>
                          <td className="px-6 py-4">
                             <div className="text-sm font-bold text-gray-900 dark:text-white">{shift.branch?.name || shift.user?.branch?.name || t('admin_shifts_page.toasts.kphehwb')}</div>
                          ...
                          <td className="px-6 py-4 font-black text-gray-900 dark:text-white">
                             {fmt(shift.totalCashSales)} <span className="text-[10px] text-gray-400 font-normal">{t('admin_shifts_page.ui.kwlxf')}</span>
                          </td>
                          <td className="px-6 py-4">
                             {shift.status === 'closed' ? (
                                <span className={`font-black tracking-wide ${shift.variance < 0 ? 'text-red-500' : shift.variance > 0 ? 'text-amber-500' : 'text-gray-400'}`} dir="ltr">
                                   {shift.variance > 0 ? '+' : ''}{fmt(shift.variance)} <span className="font-normal text-xs" dir="rtl">{t('admin_shifts_page.ui.kwlxf')}</span>
                                </span>
                             ) : (
             ...
             <div className="space-y-3 p-4 md:hidden">
               {analytics.length === 0 && !loading ? (
                 <EmptyState title={t('admin_shifts_page.titles.km3iafu')} description="لا توجد تحليلات متاحة لهذه الفترة" />
               ) : (
                 analytics.map((day, idx) => (
                   <div key={idx} className="app-surface-muted rounded-2xl p-4">
                     ...
                     <p className="mt-1 text-[11px] text-gray-400">{day.branchName || t('admin_shifts_page.toasts.kphehwb')}</p>
                     <div className="mt-3 grid grid-cols-2 gap-2">
                       <div className="rounded-xl bg-white px-3 py-2 text-center dark:bg-gray-900/70">
                         <p className="text-[10px] text-gray-400">{t('admin_shifts_page.ui.kqwyrlo')}</p>
             ...
             <table className="w-full text-sm text-right">
                <thead className="bg-gray-50/80 dark:bg-gray-800/50 text-gray-500 dark:text-gray-400 font-bold border-b border-gray-100 dark:border-gray-800">
                   <tr>
                      <th className="px-6 py-4">{t('admin_shifts_page.ui.kzbvdnf')}</th>
                      <th className="px-6 py-4">{t('admin_shifts_page.ui.kove7t8')}</th>
                      <th className="px-6 py-4">{t('admin_shifts_page.ui.kl61eln')}</th>
                      <th className="px-6 py-4">{t('admin_shifts_page.ui.kflwesj')}</th>
                      <th className="px-6 py-4">{t('admin_shifts_page.ui.kqnw030')}</th>
                      <th className="px-6 py-4">{t('admin_shifts_page.ui.kx2gnsl')}</th>
                   </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                   {analytics.length === 0 && !loading && (
                      ...
                      <tr key={idx} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                         ...
                         <td className="px-6 py-4">
                            <Badge className="bg-primary-50 text-primary-700 dark:bg-primary-900/30 dark:text-primary-400 border-primary-100">
                               {day.branchName || t('admin_shifts_page.toasts.kphehwb')}
                            </Badge>
                         </td>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-gray-900/60 backdrop-blur-md p-4 animate-fade-in" dir="rtl">
           <div className="bg-white dark:bg-gray-800 rounded-3xl w-full max-w-md overflow-hidden shadow-2xl p-6 space-y-6">
              <div className="flex items-center gap-3 text-red-600">
                 ...
                 <h3 className="text-xl font-black">{t('admin_shifts_page.ui.kmprtn')}</h3>
              </div>
              ...
              <div className="space-y-4">
                 <div>
                    <label className="block text-xs font-bold text-gray-500 mb-1.5 mr-1">{t('admin_shifts_page.ui.k7t8wdo')}</label>
                 ...
                 <div>
                    <label className="block text-xs font-bold text-gray-500 mb-1.5 mr-1">{t('admin_shifts_page.ui.kuqko64')}</label>
                    <textarea 
                       value={fcData.notes}
                       ...
                       placeholder={t('admin_shifts_page.placeholders.kit95qd')}
                    />
                 </div>
              </div>

              <div className="flex gap-3">
                 <button onClick={handleForceClose} className="flex-1 bg-red-600 hover:bg-red-700 text-white font-black py-3 rounded-2xl shadow-lg transition-all shadow-red-500/20">{t('admin_shifts_page.ui.kria2po')}</button>
                 <button onClick={() => setForceCloseModal(null)} className="px-6 py-3 bg-gray-100 hover:bg-gray-200 text-gray-600 dark:bg-gray-700 dark:hover:bg-gray-600 dark:text-white font-bold rounded-2xl transition-all">{t('admin_shifts_page.ui.cancel')}</button>
              </div>
           </div>
        </div>
      )}
