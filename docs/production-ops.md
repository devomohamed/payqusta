...
- daily JSON backup export for each active tenant
- weekly restore drill into a clean tenant or staging database
- alert review after every restore attempt
...
If `OPS_BEARER_TOKEN` is set, both commands also check protected ops routes.
...
4. Download the uploaded artifacts if you need the latest JSON snapshot or Prometheus metrics dump.
